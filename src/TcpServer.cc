...
TcpServer::TcpServer(EventLoop *base_loop, InetAddress bind_addr,
                     std::string name, Option reuse_option) 
                    :loop_(base_loop)
                    ...
                    ,pool_(std::make_shared<EventLoopThreadPool>(base_loop,name))
{
...
TcpServer::~TcpServer() 
{
    for(auto&[_,conn_ptr]:connection_map_)
    {
        ...
        conn_ptr.reset();
        //在对应的loop中删除连接
        EventLoop* ioloop=tem_conn->getLoop();
        ioloop->queueInLoop([conn=tem_conn](){conn->connectionDestroyed();});
    }
}
    ...
    TcpConnectionPtr const &tcp_connection_ptr) 
{
    ...
    //在对应loop所在的线程中执行connectionDestroyed函数，清理对应loop中连接的相关信息
    EventLoop* ioloop=tcp_connection_ptr->getLoop();
    ioloop->queueInLoop([conn=tcp_connection_ptr](){conn->connectionDestroyed();});
