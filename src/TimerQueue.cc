...
void readTimerFd(int timer_fd,Timestamp now)
{
    ...
    if(n!=sizeof(res))
    {
        LOG_ERROR("read timer_fd failed!");
    }
}
...
void resetTimerFd(int timer_fd,Timestamp expiration)
{
    ...
    LOG_DEBUG("resetTimerFd triggered!");
    if(ret == -1)
    {
        LOG_ERROR("timerfd_settime failed error:%s",std::strerror(errno));
    }
}
...
TimerQueue::TimerQueue(EventLoop *loop)
    ...
    ,timer_fd_channel(loop,timer_fd)
    ,is_running_callback_(false)
{
    //注册channel的读事件
    timer_fd_channel.setReadCallback([this](Timestamp){handleRead();});
    timer_fd_channel.enableReading();
}
...
void TimerQueue::addTimerInLoop(Timer *timer)
{
    ...
    bool is_earily_changed = insert(std::move(u_timer));

    if(is_earily_changed)
    {
...
void TimerQueue::reset(const std::vector<Entry> expired_timers, Timestamp now)
{
    ...
    //更新next_expiration设定下一次的timerfd唤醒时间
    if(!timer_list_.empty())
    {
        next_expiration = timer_list_.begin()->first.first;
    }

    if(next_expiration.vaild())
    {
        resetTimerFd(timer_fd,next_expiration);
    }
}
