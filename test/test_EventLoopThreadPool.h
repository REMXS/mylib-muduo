...
protected:
    void SetUp() override {
        ...
        baseLoop_ = std::make_unique<EventLoop>();
        pool_ = std::make_unique<EventLoopThreadPool>(baseLoop_.get(), "TestPool");
    }
...
TEST_F(EventLoopThreadPoolTest, SingleThreadMode) {
    // 设置线程数为0
    pool_->setNumThreads(0);
    auto fu=std::function<void(EventLoop*)>();
    pool_->start(fu);
...
TEST_F(EventLoopThreadPoolTest, MultiThreadMode) {
    ...
    EventLoop* loop1 = pool_->getNextLoop();
    EventLoop* loop2 = pool_->getNextLoop();
    EventLoop* loop3 = pool_->getNextLoop();
    EventLoop* loop4 = pool_->getNextLoop();
...
TEST_F(EventLoopThreadPoolTest, MultiThreadMode_EventFdNotice)
{
    ...
    */
    auto wakeUpFunc=[&](){
        std::vector<EventLoop*>subloops=pool_->getAllLoops();
    ...
    auto init_call_back=std::function<void(EventLoop*)>([&](EventLoop*el){
        call_back_trigger++;
    });
